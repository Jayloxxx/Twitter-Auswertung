"""
Update PDF export to show specific justifications instead of general descriptions
"""
...
def update_pdf_export():
    ...
    # Replace trigger table to use post-specific justifications
    old_trigger_section = """                # Full trigger descriptions
    ...
    new_trigger_section = """                # Post-specific trigger justifications
                trigger_data = [
    ...
    if old_trigger_section in content:
        content = content.replace(old_trigger_section, new_trigger_section)
        changes += 1
        print("[OK] Updated trigger table to use post-specific justifications")
    else:
        print("[WARNING] Trigger section not found")

    # Replace frame table to use post-specific justifications
    old_frame_section = """                # Full frame descriptions
                frame_descriptions = {
    ...
    new_frame_section = """                # Post-specific frame justifications
                frame_data = [
    ...
    if old_frame_section in content:
        content = content.replace(old_frame_section, new_frame_section)
        changes += 1
        print("[OK] Updated frame table to use post-specific justifications")
    else:
        print("[WARNING] Frame section not found")
...
if __name__ == '__main__':
    print("=" * 60)
    print("Updating PDF Export: Justifications Instead of Descriptions")
    print("=" * 60)
    changes = update_pdf_export()
    print("=" * 60)
    if changes > 0:
        print("[SUCCESS] PDF export updated!")
        print("\nChanges:")
        print("  - Trigger table now shows post-specific justifications")
        print("  - Frame table now shows post-specific justifications")
        print("  - Column headers changed from 'Vollstandige Beschreibung'")
        print("    to 'Begrundung fur diesen Post'")
        print("  - Shows '-' only when trigger/frame is inactive or missing justification")
        print("\nExport a new PDF to see the specific justifications!")
    else:
